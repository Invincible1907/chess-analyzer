      ...
      <main className="main-content">
        ...
        <section className="workspace-grid">
          ...
          <div className="review-panel">
            ...
            {activeView === 'review' ? <>
              <div className="review-intro"><p className="eyebrow">QUICK GAME REVIEW</p><h2>{moves ? 'Your game is ready.' : 'Bring a game to the board.'}</h2><p>{moves ? 'A fast position-by-position review with material balance and key moments.' : 'Paste a PGN or choose a game from your chess.com history.'}</p><textarea className="pgn-input" value={pgn} onChange={(event) => setPgn(event.target.value)} placeholder="Paste PGN here" rows={3} /><button className="button-primary analyze-button" onClick={() => analyzePgn()} disabled={loading}>{loading ? 'Reviewing...' : 'Review PGN'}</button></div>
              <div className="stat-grid"><div className="stat-card"><span>Material</span><strong>{review ? (review.materialScore > 0 ? `+${review.materialScore}` : review.materialScore) : '--'}</strong><small>{review ? 'white advantage' : 'waiting'}</small></div><div className="stat-card"><span>Key moments</span><strong>{review?.keyMoments ?? '--'}</strong><small>{review ? `${review.captures} captures / ${review.checks} checks` : 'not reviewed'}</small></div><div className="stat-card"><span>Moves</span><strong>{moveCount || '--'}</strong><small>{moves ? 'quick reviewed' : 'waiting'}</small></div></div>
